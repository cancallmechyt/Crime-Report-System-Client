    ...
    try {
        ...
        if (result.isConfirmed) {
            ...
            Swal.fire("บันทึกข้อมูลสำเร็จ!", "", "success").then(() => {
                window.location.reload(); // หรือ อัปเดตข้อมูลในหน้าโดยไม่ต้องรีเฟรชหน้า
            });
        } else {
...
<form>
    <div className="mt-4 grid gap-6 mb-6 md:grid-cols-2">
        <div>
            <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">รหัสนักศึกษา</label>
            <input 
              type="text" 
              value={value.usercode} 
              onChange={e => setValues({...value, usercode : e.target.value})} 
              className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" 
              required />
        </div>
        <div>
            <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">บทบาท</label>
            <select 
               id="role" 
               value={value.role} 
        ...
        <div>
            <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">คณะ / วิทยาลัย</label>
            <input 
              type="text" 
              value={value.college} 
        ...
        <div>
            <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">สาขาวิชา</label>
            <input 
              type="text" 
              value={value.major} 
    ...
    <div className="flex items-start mb-4">
        ...
        <label  className="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">ตรวจสอบแล้วว่าข้อมูลที่กรอกทั้งหมดมีความ<a className="text-blue-600 hover:underline dark:text-blue-500">ถูกต้อง</a></label>
    </div>
    <button 
    type="submit" 
