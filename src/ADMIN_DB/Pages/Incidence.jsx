    ...
    try {
      ...
      const posts = await Promise.all(
        response.data.map(async (post) => {
          ...
          const Profile =
            userDataResponse.data && userDataResponse.data[0]
              ? userDataResponse.data[0].usercode
              : "";
    ...
    <div className="p-4 sm:ml-64">
      <div className="flex justify-center items-center">
        <h1 className="text-xl font-medium text-gray-900 font-bold mr-1 mb-2">
          รายการแจ้งเหตุทั้งหมดภายในระบบ{" "}
        </h1>
      </div>
                  ...
                  {profile.map((item, index) => (
                    <div key={index} className="flex flex-col">
                      <div className="flex items-center">
                        ...
                        <div>
                          <Link to={`/list/${UID}`}>
                            <p className="text-left text-s font-bold">
                              {item.fname} {item.lname}
                            </p>
                          </Link>
                          <p className="text-s text-left text-customGrays mr-2 mb-1">
                            คณะ : {item.college}
                          </p>
                        </div>
                      </div>
