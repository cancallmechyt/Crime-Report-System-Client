...
function LocationChart() {
            ...
            try {
                const responses = await Promise.all([
                  'ตึก 1', 'ตึก 2', 'ตึก 3', 'ตึก 4', 'ตึก 5', 'ตึก 6', 'ตึก 7', 'ตึก 8', 'ตึก 9', 'ตึก 10', 'ตึก 11', 'ตึก 12', 'ตึก 13', 'ตึก 14', 'ตึก 15'
                    ...
                    const response = await useAxios.get(`/chartbar/incidence/location/${location}`);
  ...
  const filteredData = data.filter(item => item.value !== 0);
    ...
    const renderCustomizedLabel = ({ x, y, value }) => {
        return <text x={x} y={y} fill="#000" textAnchor="middle" dy={-6}>{value}</text>;
    };
  

  return (
    <div>
        <p className="flex items-center justify-center text-2xl font-bold">จำนวนเหตุทั้งหมดในสถานที่</p>
